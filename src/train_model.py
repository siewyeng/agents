...
def main(args):
    ...
    mlflow_init_status, mlflow_run = smp.general_utils.mlflow_init(
        args, setup_mlflow=args["setup_mlflow"], autolog=args["mlflow_autolog"]
    )
    smp.general_utils.mlflow_log(
        mlflow_init_status,
        "log_params",
        params={
    ...
    for epoch in range(1, args["epochs"] + 1):
        curr_train_loss = smp.modeling.utils.train(
            args, model, device, train_loader, optimiser, epoch, mlflow_init_status
        )
        curr_test_loss, curr_test_accuracy = smp.modeling.utils.test(
