...
ARG REPO_DIR="."
ARG CONDA_ENV_FILE="stemss-mini-project-conda-env.yaml"
ARG CONDA_ENV_NAME="stemss-mini-project"
...
COPY --chown=${NON_ROOT_USER}:${NON_ROOT_GID} ${REPO_DIR} stemss-mini-project
...
RUN ${CONDA_BIN} env create -f stemss-mini-project/${CONDA_ENV_FILE} && \
    ${CONDA_BIN} init bash && \
    ${CONDA_BIN} clean -a -y && \
