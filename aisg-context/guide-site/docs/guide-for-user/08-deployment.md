...
=== "Linux/macOS"
    ...
    $ cd src
    $ gunicorn stems_mini_project_fastapi.main:APP -b 0.0.0.0:8080 -w 4 -k uvicorn.workers.UvicornWorker
    ```
...
=== "Windows PowerShell"
    ...
    $ conda activate stems-mini-project
    $ cd src
    $ uvicorn stems_mini_project_fastapi.main:APP
    ```
...
`src/stems_mini_project_fastapi/config.py`. This class contains
...
`src/stems_mini_project_fastapi/config.py`:
```python
...
class Settings(pydantic.BaseSettings):

    API_NAME: str = "stems_mini_project_fastapi"
