...
=== "Linux/macOS"
    ...
    $ cd src
    $ gunicorn stemss_mini_project_fastapi.main:APP -b 0.0.0.0:8080 -w 4 -k uvicorn.workers.UvicornWorker
    ```
...
=== "Windows PowerShell"
    ...
    $ conda activate stemss-mini-project
    $ cd src
    $ uvicorn stemss_mini_project_fastapi.main:APP
    ```
...
`src/stemss_mini_project_fastapi/config.py`. This class contains
...
`src/stemss_mini_project_fastapi/config.py`:
```python
...
class Settings(pydantic.BaseSettings):

    API_NAME: str = "stemss_mini_project_fastapi"
